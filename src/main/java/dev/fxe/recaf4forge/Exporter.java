...
public class Exporter {
    ...
    private void exportResources(Path extractedMDK) {
        for (Map.Entry<String, byte[]> resources : recaf4Forge.getCurrentWorkspace().getPrimary().getFiles().entrySet()) {
            String resourcePath =
    ...
    private void exportSrc(Path extractedMDK) {
        for (Map.Entry<String, byte[]> resources : recaf4Forge.getCurrentWorkspace().getPrimary().getClasses().entrySet()) {
